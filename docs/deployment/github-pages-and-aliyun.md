...
13. 如果文本模型网关要求流式补全，可额外配置 `TEXT_AI_STREAM=true`。
14. 将函数计算和前端域名加入 `ALLOWED_ORIGINS` 白名单。
15. `api/.env` 只用于本地开发和部署注入，不要提交到 GitHub。
...
6. 当前后端采用双模型：OCR 负责识别答案，文本模型负责生成讲评建议。
7. `npm --prefix api run dev` 和 `npm --prefix api run start` 都会自动读取 `api/.env`。
8. 当前真实验收推荐使用：
   - 体验码 `demo-code`
   - 演示答案按钮自动填充答案
   - 演示答题卡 `scheme-b-filled.png`
9. 浏览器若因 OSS CORS 或预检失败无法直传，前端会自动回退到后端 `/uploads/direct/*` 代理上传；若你希望始终走浏览器直传，需要在 OSS 上正确配置 CORS。
